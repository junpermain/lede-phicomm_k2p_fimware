...
# 修改主机名字，把OpenWrt-k2p修改你喜欢的就行（不能纯数字或者使用中文）
...
# 版本号里显示一个自己的名字（1058771182 build $(TZ=UTC-8 date "+%Y.%m.%d") @ 这些都是后增加的）
sed -i "s/OpenWrt /1058771182 build $(TZ=UTC-8 date "+%Y.%m.%d") @ OpenWrt /g" package/lean/default-settings/files/zzz-default-settings
